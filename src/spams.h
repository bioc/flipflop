...
template<typename T> void _multLeftDiag(Matrix<T> *X,Matrix<T>*Y,Matrix<T> *XY) throw(const char *) {
   XY->copy((Matrix<T>&)*X);
   Vector<T> diag(Y->rawX(),X->m());
   XY->multDiagLeft(diag);
}
...
template<typename T> 
Vector<T> *_proximalPathCoding(Matrix<T> *alpha0,Matrix<T> *alpha, // graph
			       long long precision,
			       T* weights,
			       int* ir,
			       int* jc,
			       T* start_weights,
			       T* stop_weights,       // param
			       int num_threads,
			       T lambda1,
			       T lambda2,
			       bool intercept,
...
using namespace FISTA;
 ...
 param.regul = regul_from_string(name_regul);
 if (param.regul==INCORRECT_REG)
    throw("proximalPathCoding : Unknown regularization.\n  For valid names see source code of regul_from_string in spams/src/spams/prox/fista.h\n");
  strncpy(param.name_regul,name_regul,param.length_names);
  if (param.regul != GRAPH_PATH_L0 && param.regul != GRAPH_PATH_CONV && param.regul != GRAPH_PATH_CONV2)
    throw("proximalPathCoding : proximalGraph should be used instead");
  param.num_threads = (num_threads < 0) ? 1 : num_threads;
  param.lambda = lambda1;
  param.lambda2 = lambda2;
  param.intercept = intercept;
  ...
  Vector<T> *val_loss = new Vector<T>();
  FISTA::PROX<T>((Matrix<T> &)(*alpha0),(Matrix<T> &)(*alpha),param,(Vector<T> &)(*val_loss),NULL,NULL,&graph);
  return val_loss;
}

template<typename T> 
Matrix<T> *_fistaPathCoding(Matrix<T> *X,AbstractMatrixB<T> *D,Matrix<T> *alpha0,
	     Matrix<T> *alpha,  
             long long precision,
             T* weights,
             int* ir,
...
using namespace FISTA;
 ...
 int nAlpha = alpha0->n();
  ...
  if (param.regul==INCORRECT_REG) {
      throw("fistaPathCoding: Unknown regularization.\n  For valid names see source code of regul_from_string in spams/src/spams/prox/fista.h\n");
  }
  strncpy(param.name_regul,name_regul,param.length_names);
  strncpy(param.name_loss,name_loss,param.length_names);
  param.verbose = verbose;
  ...
  if (param.regul==TREE_L0 || param.regul==TREEMULT || param.regul==TREE_L2 || param.regul==TREE_LINF) 
      throw("Error: fistaFlat should be used instead");
  if (param.regul != GRAPH_PATH_L0 && param.regul != GRAPH_PATH_CONV && param.regul != GRAPH_PATH_CONV2)
     throw("fistaPathCoding : another proximal function should be used instead");
  ...
  Matrix<T> *duality_gap = new Matrix<T>();
  //FISTA::solver((Matrix<T> &)(*X),(AbstractMatrixB<T> &)(*D),(Matrix<T> &)(*alpha0),(Matrix<T> &)(*alpha),param,(Matrix<T> &)(*duality_gap));
  FISTA::solver<T>((Matrix<T> &)(*X),(AbstractMatrixB<T> &)(*D),(Matrix<T> &)(*alpha0),(Matrix<T> &)(*alpha),param,(Matrix<T> &)(*duality_gap),NULL,NULL,&graph);
  return duality_gap;
}
...
{  
  ...
  FISTA::poissonSolver((Vector<T>&)*y,(SpMatrix<T>&)*X,(Vector<T>&)*beta0,(Vector<T>&)*beta,(Vector<T>&)*weights,delta,max_iter,tol);
}


template<typename T>
void _solverPoissonFull(
      Vector<T> *y, 
      Matrix<T>* X,
      Vector<T>* beta0,
      Vector<T>* beta,
...
{  
  ...
  FISTA::poissonSolver((Vector<T>&)*y,(Matrix<T>&)*X,(Vector<T>&)*beta0,(Vector<T>&)*beta,(Vector<T>&)*weights,delta,max_iter,tol);
}



template<typename T>
SpMatrix<T> *_sepCostsPathCoding(Matrix<T> *alpha0, // graph
				 Matrix<T>* alpha,
				 T* weights,
				 int* ir,
				 int* jc,
...
using namespace FISTA;
  ...
  param.regul = regul_from_string(name_regul);
  if (param.regul==INCORRECT_REG)
     throw("sepCostsPathCoding : Unknown regularization.\n  For valid names see source code of regul_from_string in spams/src/spams/prox/fista.h\n");
  strncpy(param.name_regul,name_regul,param.length_names);
  strncpy(param.name_loss,name_loss,param.length_names);
  const loss_flow loss=loss_flow_from_string(param.name_loss);
//  print_loss_flow(loss);
  strncpy(param.name_loss,name_loss,param.length_names);
  param.num_threads = (num_threads < 0) ? 1 : num_threads;
  param.lambda = lambda;
  param.tol = tol;
  param.delta = delta;
  param.pos= pos;
  param.inner_weights=loss_weights;
  int pAlpha = alpha0->m();
  ...
  graph.epsilon_flow = epsilon_flow;
  graph.mode_decomposition=mode_decomposition;

  //  graph.prices=NULL;
  //  if (prices->n() > 1) {
  graph.prices =  prices->rawX();
  //  }

  SpMatrix<T> *path_out = new SpMatrix<T>();
...
template<typename T>
SpMatrix<T> *_evalPathCoding(Matrix<T> *alpha0, 
			     Vector<T> *dual_val,
			     long long precision,
			     T* weights,            // graph
			     int* ir,
...
using namespace FISTA;
 ...
 param.regul = regul_from_string(name_regul);
 if (param.regul==INCORRECT_REG)
    throw("evalGraphPath : Unknown regularization.\n  For valid names see source code of regul_from_string in spams/src/spams/prox/fista.h\n");
  strncpy(param.name_regul,name_regul,param.length_names);
  if (param.regul != GRAPH_PATH_L0 && param.regul != GRAPH_PATH_CONV && param.regul != GRAPH_PATH_CONV2)
    throw("evalGraphPath : evalGraph should be used instead");
  param.num_threads = (num_threads < 0) ? 1 : num_threads;
  param.lambda = lambda1;
  param.lambda2 = lambda2;
  param.intercept = intercept;
  ...
  param.clever = clever;
  param.eval = eval;
  param.eval_dual_norm = eval_dual;  
  param.new_solver=true;
  param.size_group = size_group;
  param.transpose = transpose;
  if (param.num_threads == -1) {
  ...
  Vector<T> val;
  SpMatrix<T> *path = new SpMatrix<T>();

  //FISTA::EvalGraphPath<T>((Matrix<T> &)(*alpha0),param,val,&graph,path);
  if(eval_dual)
    {       
      FISTA::EvalGraphPath<T>((Matrix<T> &)(*alpha0),param,(Vector<T> &)(*dual_val),&graph,path);
    }
  else
    {
      FISTA::EvalGraphPath<T>((Matrix<T> &)(*alpha0),param,val,&graph,path);
...
SpMatrix<T> *_removeCyclesGraph(int mD, int p, double *D_v, int *D_r, int *D_pB)  throw(const char *){
  ...
  SpMatrix<T> G(D_v,D_r,D_pB,D_pE,mD,p,D_pB[p]);
  
  SpMatrix<T> *G2 = new SpMatrix<T>();

  remove_cycles(G,(SpMatrix<T> &)(*G2));
