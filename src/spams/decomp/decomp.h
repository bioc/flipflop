...
void coreLARS2(Vector<T>& DtR, const AbstractMatrix<T>& G,
      ...
      T* pr_path = NULL, int length_path = -1, const bool safe = true);
...
void coreLARS2(Vector<T>& DtR, const AbstractMatrix<T>& G,
      ...
      const T constraint,
      const bool pos,
      T* path, int length_path, const bool safe) {
   const int LL = Gs.n();
   const int K = G.n();
   const int L = MIN(LL,K);
   if (length_path <= 1) length_path=4*L;
 ...
 //     coeffs.print("coeffs");
      if (safe) {
         for (int j = 0; j<2*K; ++j) 
            if (pr_work[j] < 1e-15) pr_work[j]=INFINITY;
      }
