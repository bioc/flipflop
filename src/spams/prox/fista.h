...
namespace FISTA {
   ...
   regul_t regul_from_string(char* regul) {
      ...
      if (strcmp(regul,"graph-path-conv2")==0) return GRAPH_PATH_CONV2;
...
//            virtual bool is_pos() const { return _pos; };
   ...
   template <typename T>
    ...
    template <typename T,typename Int>
       inline void convert_paths_to_mat(const List<Path<Int>*>& paths,SpMatrix<T>& paths_mat, const int n) {
          int nzmax=0;
          for (ListIterator<Path<Int>*> it=paths.begin(); it != paths.end(); ++it)
             nzmax+=it->nodes.size();
          ...
          if (paths.size() > 0)
             pB[0]=0;
          for (ListIterator<Path<Int>*> it_path=paths.begin(); 
       ...
       class GraphPathL0 : public Regularizer<T> {
          public:
             ...
             T inline eval_paths(const Vector<T>& x, SpMatrix<T>& paths_mat) const { 
                ...
                convert_paths_to_mat<T,long long>(paths,paths_mat,_graph.n());
       ...
       class GraphPathConv : public Regularizer<T> {
          public:
             GraphPathConv(const ParamReg<T>& param) : Regularizer<T>(param) {
                const GraphPathStruct<T>& graph=*(param.graph_path_st);
                _new_solver=param.new_solver;
                if (_new_solver) {
                   _graph2.init_graph(graph);
                } else {
                   _graph.init_graph(graph);
                }
                _alt_norm=param.alt_norm;
             //   if (_alt_norm) this->_pos=true;
             }
             ...
             void inline prox(const Vector<T>& x, Vector<T>& y, const T lambda) {
                y.copy(x);
                //if (this->_pos) y.thrsPos();
                if (this->_pos && !_alt_norm) y.thrsPos();
                if (!this->_pos) y.abs_vec();
               // y.print("y");
                if (_new_solver) {
                   _graph2.proximal_conv(y.rawX(),lambda,_alt_norm);
                } else {
                   if (this->_pos) y.thrsPos();
                   _graph.proximal_conv(y.rawX(),lambda,_alt_norm);
                }
                if (!this->_pos)
                   for (int i = 0; i<y.n(); ++i) 
                      if (x[i] < 0) y[i]=-y[i];
             ...
             T inline eval(const Vector<T>& x) const { 
                if (_new_solver) {
                   ...
                   return const_cast<GraphPath<T>* >(&_graph)->eval_conv(x.rawX(),NULL,_alt_norm);
                }
             };
             T inline eval_dual_norm(const Vector<T>& x) const { 
                if (_new_solver) {

                   return const_cast<DoubleGraphPath<T>* >(&_graph2)->eval_dual_norm(x.rawX(),NULL,_alt_norm);
                } else {
                   return const_cast<GraphPath<T>* >(&_graph)->eval_dual_norm(x.rawX(),NULL,_alt_norm);
                }
             };
             T inline eval_paths(const Vector<T>& x, SpMatrix<T>& paths_mat) const { 
                T val;
                if (_new_solver) {
                   List<Path<T>*> paths;
                   val=const_cast<DoubleGraphPath<T>* >(&_graph2)->eval_conv(x.rawX(),&paths,_alt_norm);
                   convert_paths_to_mat<T,T>(paths,paths_mat,_graph2.n());
                   for (ListIterator<Path<T>*> it_path=paths.begin(); 
                         it_path != paths.end(); ++it_path) delete(*it_path);
                } else {
                   List<Path<long long>*> paths;
                   val=const_cast<GraphPath<T>* >(&_graph)->eval_conv(x.rawX(),&paths,_alt_norm);
                   convert_paths_to_mat<T,long long>(paths,paths_mat,_graph.n());
             ...
             T inline eval_dual_norm_paths(const Vector<T>& x, SpMatrix<T>& paths_mat) const {
                ...
                if (_new_solver) {
                   Path<T> path;
                   val=const_cast<DoubleGraphPath<T>* >(&_graph2)->eval_dual_norm(x.rawX(),&path.nodes,_alt_norm);
                   List<Path<T>*> paths;
                   paths.push_back(&path);
                   path.flow_int=T(1.0);
                   path.flow=double(1.0);
                   convert_paths_to_mat<T>(paths,paths_mat,_graph2.n());
                } else {
                   Path<long long> path;
                   val=const_cast<GraphPath<T>* >(&_graph)->eval_dual_norm(x.rawX(),&path.nodes,_alt_norm);
                   List<Path<long long>*> paths;
                   paths.push_back(&path);
                   path.flow_int=1;
                   path.flow=double(1.0);
             ...
             virtual bool is_fenchel() const { return _alt_norm ? this->_pos : true; };

             void inline fenchel(const Vector<T>& input, T& val, T& scal) const {
                T mm;
                if (this->_pos && !_alt_norm) {
                   Vector<T> output;
                   output.copy(input);
                   output.thrsPos();
                   if (_new_solver) {
                      mm = const_cast<DoubleGraphPath<T>* >(&_graph2)->eval_dual_norm(output.rawX(),NULL,_alt_norm);
                   } else {
                      mm = const_cast<GraphPath<T>* >(&_graph)->eval_dual_norm(output.rawX(),NULL,_alt_norm);
                   }
                } else {
                   if (_new_solver) {
                      mm = const_cast<DoubleGraphPath<T>* >(&_graph2)->eval_dual_norm(input.rawX(),NULL,_alt_norm);
                   } else {
                      mm = const_cast<GraphPath<T>* >(&_graph)->eval_dual_norm(input.rawX(),NULL,_alt_norm);
                   }
                }
                scal= mm > 1.0 ? T(1.0)/mm : 1.0;
                val=0;
...
#pragma omp critical
   ...
   template <typename T, typename D, typename E>
      void ISTA_Generic(Loss<T,D,E>& loss, Regularizer<T,D>& regularizer, const D& x0, D& x, Vector<T>& optim_info,
         ...
         const T alphamin=10e-15*alphamax;
   ...
   template <typename T>
      Regularizer<T>* setRegularizerVectors(const ParamFISTA<T>& param,
         ...
         param_reg.new_solver=param.new_solver;
         Regularizer<T>* reg;
         switch (param.regul) {
...
#pragma omp parallel for private(i) 
   ...
   template <typename T>
      void SepLossPathCoding(const Matrix<T>& alpha0,
            Matrix<T>& alpha,
            const ParamFISTA<T>& param, const loss_flow loss,
            const GraphPathStruct<T>* graph_path_st, 
            SpMatrix<T>* paths_mat = NULL) {
         ...
         num_threads=init_omp(num_threads);
         const int M = alpha0.n();
         DoubleGraphPath<T>** graphs = new DoubleGraphPath<T>*[num_threads];
         for (int i = 0; i<num_threads; ++i) {
         ...
         if (!alt_norm) 
            switch (loss) {
               case FLOW_SQUARE : new_loss=FLOW_SQUARE_POS; break;
...
#pragma omp parallel for private(i) 
         for (i = 0; i< M; ++i) {
            ...
            graphs[numT]->sep_costs_conv(variables,param.lambda,new_loss,(i==0 && paths_mat == NULL) ? NULL : &paths,weights,delta,param.tol);


            if (new_loss==FLOW_SQUARE_POS) 
               for (int j=0; j<a.n(); ++j) a[j]=MIN(a[j],old_a[j]);
            if (new_loss==FLOW_SQUARE_POS_WEIGHTS) 
               for (int j=0; j<a.n(); ++j) a[j]=MIN(a[j],old_a[j]/weights[j]);
            if (new_loss==FLOW_POISSON_POS) 
               for (int j=0; j<a.n(); ++j) a[j]=MAX(MIN(a[j],old_a[j]-delta),0);
            if (new_loss==FLOW_POISSON_WEIGHTS_POS) 
               for (int j=0; j<a.n(); ++j) a[j]=MAX(MIN(a[j],old_a[j]/weights[j]-delta),0);
            if (i==0 && paths_mat) {
               convert_paths_to_mat<T,T>(paths,*paths_mat,graphs[numT]->n());
               for (ListIterator<Path<T>*> it_path=paths.begin(); 
                     it_path != paths.end(); ++it_path) delete(*it_path);
            } 
            if (!pos) {
         ...
         for (int i = 0; i<num_threads; ++i) delete(graphs[i]);
         delete[](graphs);
      };

   template <typename T, typename U>
      void poissonSolver(const Vector<T>& y, U& X,
            const Vector<T>& w0, Vector<T>& w, const Vector<T>& eta0, const T delta,
            const int max_it, const T tol) {
         const int p=X.n();
         const int n=y.n();
         typename U::col col;
         if (X.n() == 0) return;
         ...
         Vector<T> ones(X.m());
         ones.set(T(1.0));
         X.multTrans(ones,eta,1.0,1.0);

         T mean_eta=eta.sum();
         T mean_eta0=eta0.sum();
         ...
         tabmu[7]=0.001;
         tabmu[8]=0.0001;
         tabmu.scal(((mean_eta)/(mean_eta-mean_eta0)));
         Vector<T> tabscores(tabmu.n());
         tabscores.setZeros();
         ...
         for (int i = 0; i<X.n(); ++i) {
            Xcopy.refCol(i,col);
            col.scal(T(1.0)/eta[i]);
            mean_nrm += col.nrm2();
         }
         mean_nrm /= X.n();
         Xcopy.scal(T(1.0)/mean_nrm);
         ...
         for (int i=0; i<tabmu.n()+1; ++i) {
            ...
            for (int j = 0; j< num_samples; ++j) {
               if ( (j+1) % 5 == 0) {
                  wold.sub(w);
                  zold.sub(z);
                  if ((wold.nrm2()/(w.nrm2()+1e-10) +(zold.nrm2()/(z.nrm2()+1e-10))) < tol)
                     break;
               }
               wold.copy(w);
               zold.copy(z);
               ...
               const T gamma=yy.nrm2(); 
               yy.scal(T(1.0)/gamma);
               const T lambda=T(1.0)/(mean_nrm*gamma*mu);

               // solve lasso problem
               //   min_w 0.5|| yy- Xw ||^2 + sum_j (eta_j/mu) | w_j|
               // equivalent to
               //   min_w 0.5|| yy'  - X' w'||^2 + lambda | w'|_1
               //   
               // Guess the Lasso solution and check optimality conditions

               bool optlasso1, optlasso2;
               optlasso1 = optlasso2 =true;
               if (nnz > 0) {
                  for (int k = 0; k < nnz; ++k) {
                     const int ind=indices[k];
                     Xcopy.refCol(ind,col);
                     wtest2[k]= col.dot(yy)- lambda * signs[k];
                  }
                  invXtX.mult(wtest2,wtest); 
                  for (int k=0; k< nnz; ++k) {
                     if (signs[k]*wtest[k] < 0) {
                        optlasso1=false;
                        break;
                     }
                  }
               ...
               if (optlasso1 && optlasso2) {
                  w.setZeros();
                  for (int k=0; k< nnz; ++k) {
                     const int ind=indices[k];
                     w[ind]=gamma*wtest[k] / (mean_nrm*eta[ind]);
                  }
                  zz.scal(gamma);
               } else {
                  T normX=T(1.0);
                  wtest.setn(p);
                  indices.setn(p);
                  Xcopy.multTrans(yy,DtR);
                  coreLARS2(DtR,G,Gs,Ga,invGs,u,wtest,indices,work,normX,PENALTY,lambda,false);
                  nnz=0;
                  for (int k=0; k<p; ++k) {
                     if (indices[k] == -1) break;
                     ++nnz;
                  }
                  wtest.setn(nnz);
                  wtest.scal(gamma);
                  for (int k=0; k<nnz; ++k)
                     signs[k]= wtest[k] > 0 ? 1 : -1;
                  wtest2.setn(nnz);
                  invXtX.setn(nnz);
                  invXtX.setm(nnz);
                  for (int k=0; k<nnz; ++k)
                     for (int l=k; l<nnz; ++l) 
                        invXtX(k,l)=invGs(k,l);
                  invXtX.fillSymmetric();
                  zz.setZeros();
                  for (int k = 0; k < nnz; ++k) {
                     const int ind=indices[k];
                     Xcopy.refCol(ind,col);
                  ...
                  for (int k=0; k< nnz; ++k) {
                     const int ind=indices[k];
                     w[ind]=wtest[k] / (mean_nrm*eta[ind]);
                  }
               }
            ...
            if (i < tabmu.n()) {
               tmp.copy(w);
               tmp.mult(eta,tmp);
               tabscores[i]=tmp.asum();
               b.copy(zz);
               b.add(delta);
               b.log();
               b.mult(y,b);
               tabscores[i] -= b.sum();
               if (i > 0 && tabscores[i] > tabscores[i-1]) {
                  for ( ; i<tabscores.n()-1; ++i) tabscores[i+1]=tabscores[i]; 
               }
            }
         }
         w.thrsPos();
