...
#define MAX_FLOW 1e30
#define MAX_CAPACITY 1e10
#define MAX_DEMAND 1e25
#define EPSILON_FLOW 1e-10
#define TOL_EPS 1e-5
...
template <typename T> struct GraphPathStruct { 
   GraphPathStruct() { ir=NULL; jc=NULL; n=0; m=0;
      ...
      start_weights=NULL; stop_weights=NULL; epsilon_flow=EPSILON_FLOW;
      max_capacity=MAX_CAPACITY; prices=NULL; mode_decomposition=1;
      };
      mwSize* ir;
      mwSize* jc;
      int n;
...
template <typename T = double, typename Int = long long> class GraphPath {
   public:
      ...
      T eval_conv(const T* variables, List<Path<Int>*>* decomposition = NULL, const bool alt_norm = false);
      T eval_dual_norm(const T* variables, list_int* path_out = NULL, const bool alt_norm = false);
      void proximal_l0(T* variables, const T lambda);
      void proximal_conv(T* variables,const T lambda, const bool alt_norm = false);
...
//   _num_fixed=graph.num_fixed;
   ...
   T max_weight=weights.fmaxval();
   for (int i = 0; i< _n; ++i) {
      max_weight= isinf(_init_start_weights[i]) ? max_weight : MAX(max_weight,_init_start_weights[i]);
      max_weight= isinf(_init_stop_weights[i]) ? max_weight : MAX(max_weight,_init_stop_weights[i]);
   }
   ...
   for (int i = 0; i<_n; ++i) {
      //if (!isinf(graph.start_weights[i])) {
         ...
         _min_cost_flow->add_edge(2*_n,i,cost,double_cost,isinf(graph.start_weights[i]) ? 0 : _infinite_capacity);
     // }
   }
   // j'-t connexions
   for (int i = 0; i<_n; ++i) {
//      if (!isinf(graph.stop_weights[i])) {
         ...
         _min_cost_flow->add_edge(i+_n,2*_n+1,cost,double_cost,isinf(graph.stop_weights[i]) ? 0 : _infinite_capacity);
//      }
   }
   // other connexions
   for (int i = 0; i<_n; ++i) {
...
T GraphPath<T,Int>::eval_conv(const T* variables, List<Path<Int>*>* decomposition, const bool alt_norm) {
   ...
   if (alt_norm) {
      for (int i = 0; i<_n; ++i) {
         _min_cost_flow->set_edge(i,0,0,0);
      }
   }
   _min_cost_flow->compute_min_cost(false,false);
   const T val = static_cast<T>(_min_cost_flow->compute_cost_double())/(2*_sf*_sf);
...
template <typename T, typename Int>
T GraphPath<T,Int>::eval_dual_norm(const T* variables, list_int* path_out, const bool alt_norm) {
   T tau=T(1.0);
   list_int path;
   ...
   if (path_out) {
      path_out->clear();
      for (const_iterator_int it = path.begin(); it != path.end(); ++it) 
         if (*it  < _n) path_out->push_back(*it);
   }
   return tau;
};
...
template <typename T, typename Int>
void GraphPath<T,Int>::proximal_conv(T* variables, const T lambda, const bool alt_norm) {
...
enum loss_flow { FLOW_LINEAR, FLOW_SQUARE, FLOW_SQUARE_POS, FLOW_SQUARE_WEIGHTS, FLOW_SQUARE_POS_WEIGHTS, FLOW_POISSON, FLOW_POISSON_POS, FLOW_POISSON_WEIGHTS, FLOW_POISSON_WEIGHTS_POS, INCORRECT_LOSS_FLOW };

loss_flow loss_flow_from_string(char* regul, const bool alt_norm = false) {
   if (strcmp(regul,"linear")==0) return FLOW_LINEAR;
   if (strcmp(regul,"square")==0) return FLOW_SQUARE;
   if (strcmp(regul,"square-weighted")==0) return FLOW_SQUARE_WEIGHTS;
   if (strcmp(regul,"poisson")==0) return FLOW_POISSON;
   if (strcmp(regul,"poisson-weighted")==0) return FLOW_POISSON_WEIGHTS;
   return INCORRECT_LOSS_FLOW;
};
...
template <typename T> struct LossFlowParam {
   LossFlowParam() { _weights=NULL; _delta=0; _y=NULL; _loss=FLOW_LINEAR; };
   T* _weights;
   T  _delta;
   T* _y;
   loss_flow _loss;
...
template <typename T> class DoubleMinCostFlow {
   public:
      DoubleMinCostFlow(const int n, const int* num_arcs, const T epsilon_flow=EPSILON_FLOW);
      ~DoubleMinCostFlow();

      void inline add_edge(const int u, const int v, const T cost, const T cap);
      void inline set_edge(const int u, const int v, const T cost, const T cap); 
      void inline set_edge(const int u, const int v, const T cost, const T cap, const loss_flow loss, const T param2 = 0, const T param3 = 0); 
      void inline set_demand(const int node, const T dem) { _demand[node]=dem; };
      void inline add_flow(const int node, const int num_arc, const T flow); 
      void inline set_node_s(const int node) { _demand[node]=-MAX_DEMAND; _node_s=node; };
      void inline set_node_t(const int node) { _demand[node]=MAX_DEMAND; _node_t=node; };
      T inline compute_cost() const;
      ...
      void inline st_flow_decomposition_dag(List<Path<T>*>& list, const int s, const int t) const;
      void inline st_flow_decomposition_dag2(List<Path<T>*>& list, const int s, const int t, const int mode = 1) const;
      T cost_shortest_path_in_dag(list_int& path);
      void inline scale_costs(const T scal);

   protected:
      T inline eval_flow_arc(const int node, const int arc) const;
      T inline eval_flow_derivative_arc(const int node, const int arc) const;
      T inline eval_flow_saturate_arc(const int node, const int arc, const T target) const;
      T inline reduced_cost(const int node, const int child, const int arc) const;
      void inline refine(const T eps);
      void inline saturate_arc(const int node, const int arc);
      void inline discharge(const int node, const T eps);
      T inline check_reduced_costs() const;
      void inline price_update(const T eps);
      bool inline topological_sort(const bool admissible = false, bool* admiss = NULL, T* rcosts = NULL);
      void inline check_balance_excess();
...
template <typename T>
DoubleMinCostFlow<T>::DoubleMinCostFlow(const int n, const int* max_num_arcs, const T epsilon_flow) {
   _epsilon_flow=epsilon_flow;
   _n=n;
   _m=0;
   _alpha=2.0; // 16 before with price_heuristic
   _total_demand=MAX_FLOW;

   _prices=new T[n];
   memset(_prices,0,n*sizeof(T));
   _excess=new T[n];
   memset(_excess,0,n*sizeof(T));
   _demand=new T[n];
   memset(_demand,0,n*sizeof(T));
   _active=new bool[n];
   memset(_active,false,n*sizeof(bool));
   _num_arcs=new int[n];
   memset(_num_arcs,0,n*sizeof(int));
   ...
   memset(_reverse,-1,_maxm*sizeof(int));
   _flow=new T[_maxm];
   memset(_flow,0,_maxm*sizeof(T));
   _capacity=new T[_maxm];
   memset(_capacity,0,_maxm*sizeof(T));
   _save_capacity=new T[_maxm];
   memset(_save_capacity,0,_maxm*sizeof(T));
   _cost=new T[_maxm];
   memset(_cost,0,_maxm*sizeof(T));
   _loss=new loss_flow[_maxm];
   memset(_loss,FLOW_LINEAR,_maxm*sizeof(loss_flow));
   _param_loss2=new T[_maxm];
   memset(_param_loss2,0,_maxm*sizeof(T));
   _param_loss3=new T[_maxm];
   memset(_param_loss3,0,_maxm*sizeof(T));
   _topological_order=new int[n];
   memset(_topological_order,0,n*sizeof(int));
   _topologically_sorted=false;
};
...
template <typename T>
DoubleMinCostFlow<T>::~DoubleMinCostFlow() {
   delete[](_prices);
   delete[](_active);
   delete[](_excess);
   delete[](_demand);
...
template <typename T>
void inline DoubleMinCostFlow<T>::add_edge(const int u, const int v, const T cost, const T cap) {
//   cout << "add edge " << u << " " << v << " " << cost << " " << cap << " " << endl;
//   flush(cout);
   const int pu=_pr_node[u];
   const int pv=_pr_node[v];
   const int nu=_num_arcs[u]+pu;
   const int nv=_num_arcs[v]+pv;
//   cout << "add edge " << nu << " " << nv << endl;
   _children[nu]=v;
   _children[nv]=u;
   _capacity[nu]=cap;
   _capacity[nv]=0;
...
template <typename T>
void inline DoubleMinCostFlow<T>::set_edge(const int u, const int v, const T cost, const T cap) {
   const int pu=_pr_node[u];
   const int nu=pu+v; 
   _cost[nu]=cost;
   _capacity[nu]=cap;
...
template <typename T>
void inline DoubleMinCostFlow<T>::set_edge(const int u, const int v, const T cost, const T cap, 
      const loss_flow loss, const T param2, const T param3) {
   const int pu=_pr_node[u];
   const int nu=pu+v; 
   const int rev=_reverse[nu];
   _cost[nu]=cost;
   _cost[rev]=cost;
   _capacity[rev]=0;
   _capacity[nu]=cap;
   _loss[nu]=loss;
   _loss[rev]=loss;
   _param_loss2[nu]=param2;
   _param_loss2[rev]=param2;
   _param_loss3[nu]=param3;
   _param_loss3[rev]=param3;
};
...
template <typename T>
T inline DoubleMinCostFlow<T>::compute_cost() const {
   T cost=0;
   for (int i = 0; i<_n; ++i) 
      for (int j = 0; j<_num_arcs[i]; ++j)
...
T inline DoubleMinCostFlow<T>::eval_flow_arc(const int node, const int arc) const {
   ...
   T tmp;
   switch (loss) {
      case FLOW_LINEAR: tmp= param1*flow;break;
      case FLOW_SQUARE: tmp = param1-flow; tmp=0.5*tmp*tmp; break;
      case FLOW_SQUARE_POS: tmp = MAX(param1-flow,0); tmp=0.5*tmp*tmp; break;
      case FLOW_SQUARE_WEIGHTS: tmp = param1-param2*flow; tmp=0.5*tmp*tmp; break;
      case FLOW_SQUARE_POS_WEIGHTS: tmp = MAX(param1-param2*flow,0); tmp=0.5*tmp*tmp; break;
      case FLOW_POISSON: tmp = param3+flow; tmp=tmp-param1*alt_log<T>(tmp); break;
...
template <typename T>
T inline DoubleMinCostFlow<T>::reduced_cost(const int node, const int child, const int arc) const {
   return this->eval_flow_derivative_arc(node,arc)-_prices[child]+_prices[node];
};

template <typename T>
T inline DoubleMinCostFlow<T>::eval_flow_derivative_arc(const int node, const int arc) const {
   const int pointer=_pr_node[node]+arc;
   const T& param1=_cost[pointer];
   const T& param2=_param_loss2[pointer];
   const T& param3=_param_loss3[pointer];
   ...
   switch (loss) {
      ...
      case FLOW_SQUARE: tmp = flow-param1; break;
      case FLOW_SQUARE_POS: tmp = flow-param1; if (tmp > 0) tmp = 0; break;
      case FLOW_SQUARE_WEIGHTS: tmp = param2*(param2*flow-param1); break;
      case FLOW_SQUARE_POS_WEIGHTS: tmp = -param2*MAX(param1-param2*flow,0); break;
      case FLOW_POISSON: tmp = T(1.0)-param1/(param3+flow); break; 
...
template <typename T>
T inline DoubleMinCostFlow<T>::eval_flow_saturate_arc(const int node, const int arc, const T old_target) const {
   const int pointer=_pr_node[node]+arc;
   const T& param1=_cost[pointer];
   const T& param2=_param_loss2[pointer];
   const T& param3=_param_loss3[pointer];
   const loss_flow& loss=_loss[pointer];
   const T target=_capacity[pointer] > 0 ? old_target : -old_target;
   T tmp;
   switch (loss) {
      case FLOW_LINEAR: tmp = param1 <= target ? MAX_FLOW : 0; break;
      case FLOW_SQUARE: tmp = target+param1; break;
      case FLOW_SQUARE_POS: tmp = target > 0 ? MAX_FLOW : target+param1; break;
      case FLOW_SQUARE_WEIGHTS: tmp = (target+param1*param2)/(param2*param2); break;
      case FLOW_SQUARE_POS_WEIGHTS: tmp = target > 0 ? MAX_FLOW : (target+param1*param2)/(param2*param2); break;
      case FLOW_POISSON: tmp =target >= T(1.0) ? MAX_FLOW : MAX(param1/(T(1.0)-target)-param3,0); break;
...
template <typename T>
void inline DoubleMinCostFlow<T>::solve_min_cost(const T tol, const bool price_restart) {
   T eps=0;
   _time1.reset();
   _time1.start();
   _time2.reset();
   ...
   for (int i=0; i< _maxm; ++i) {
      if (_cost[i] > eps) eps=_cost[i];
   }
   if (price_restart)
      memset(_prices,0,_n*sizeof(T));
   memset(_flow,0,_maxm*sizeof(T));
   memset(_active,false,_n*sizeof(bool));
   for (int i=0; i<_n; ++i) _excess[i]=-_demand[i];
   _excess[_node_s]=_total_demand;
   _excess[_node_t]=-_total_demand;
   memcpy(_save_capacity,_capacity,_maxm*sizeof(T));
   for (int i=0; i<_maxm; ++i) _capacity[i]=MIN(_capacity[i],_total_demand);
   for (int i=0; i<_num_arcs[_node_s]; ++i)
      if (_children[_pr_node[_node_s]+i]==_node_t)
         _capacity[_pr_node[_node_s]+i]=_excess[_node_s];
...
//   cout << " epsilon-optimality: ";
   while (true) {
      // center the prices
      T mean_price=0;
      for (int i = 0; i<_n; ++i) mean_price+=_prices[i];
      mean_price/=_n;
      for (int i = 0; i<_n; ++i) _prices[i] -= mean_price;
...
//      for (int i=0; i<_n; ++i) _excess[i]=0; // remove small residuals
 //     this->print_excess();
//      flush(cout);
  //    _time1.stop();
  //    cout << "eps " << eps << endl;
  ...
  //    stop();
  //    _time1.start();
      eps = MAX(eps/_alpha,tol-_epsilon_flow);
   }
//   this->check_reduced_costs();
//   cout << "exit" << endl;
//   cout << endl;
//   flush(cout);
   memcpy(_capacity,_save_capacity,_maxm*sizeof(T));
   _time1.stop();

//   cout << "Num pushes: " << num_pushes << ", num relabels: " << num_relabels << endl;
//   _time1.printElapsed();
//   cout << "Time for price updates" << endl;
//   _time2.printElapsed();
//   flush(cout);
//   flush(cerr);
};
...
template <typename T>
void inline DoubleMinCostFlow<T>::refine(const T eps) {
   // find a pseudo-flow satisfying epsilon-opt
// cout << endl;
// cout << endl;
// cout << endl;
//cout << "Before saturation: " << eps << endl;
// this->print_excess();
// this->check_reduced_costs();
   for (int i = 0; i<_n; ++i) 
      for (int j = 0; j<_num_arcs[i]; ++j)
...
//   stop();
   ...
   for (int i = 0; i<_n; ++i)
      if (_excess[i] > _epsilon_flow && !_active[i]) {
         _list_active.push_back(i);
         _active[i]=true;
      }

   long long num_discharge=0;
   while (!_list_active.empty()) {
      ++num_discharge;
      if (price_heuristic && (_time2.getElapsed()/_time1.getElapsed() < 0.5)) {
        // cout << "Before" << endl;
        // _time1.stop();
        // this->check_reduced_costs();
        // _time1.start();
         this->price_update(eps);
        // _time1.stop();
        // cout << "After" << endl;
        // this->check_reduced_costs();
         //flush(cout);
...
template<typename T>
void inline DoubleMinCostFlow<T>::discharge(const int node, const T eps) {
//   cout << "Discharge node " << node << " with excess " << _excess[node] << endl;
 //  flush(cout);
 //  this->print_excess();
//   this->print_prices();
//   stop();
   if (_excess[node] <= _epsilon_flow) return;

//  if (_excess[node] < 1e-8)
//      this->print_excess();
   // sequence of pushes
//   T max_cmp_cost=-MAX_FLOW;
//   int old_pushes=num_pushes; 
   for (int j = 0; j<_num_arcs[node]; ++j) {
      const int pointer=_pr_node[node]+j;
      const T cap_residual=_capacity[pointer]-_flow[pointer];
      const int child=_children[pointer];
//      cout << "arc : " << j << " flow " << _flow[pointer] <<  " residual " << cap_residual << endl;
      //if (cap_residual > _epsilon_flow) {
      if (cap_residual > 0) {
         const T target=_prices[child]-_prices[node];
         const T reduced_cost=this->eval_flow_derivative_arc(node,j)-target;
//         cout << " Reduced cost " << reduced_cost << endl;
         if (reduced_cost < 0) {
            const T saturated_amount = this->eval_flow_saturate_arc(node,j,target);
            const T delta = saturated_amount-_flow[pointer];
            //if (delta > _epsilon_flow) {
            if (delta > 0) {
               num_pushes++;
  //             cout << delta << " " << cap_residual << " " << _excess[node] << endl;
               if (cap_residual < _excess[node]) {
                  if (saturated_amount >= _capacity[pointer]) {
                     _excess[node]-=cap_residual;
                     _excess[child]+=cap_residual;
                     _flow[pointer]=_capacity[pointer];
                     _flow[_reverse[pointer]] = -_capacity[pointer];
    //                 cout << "Push " << cap_residual << endl;
                  } else {
                     _excess[node] -=delta;
                     _excess[child]+=delta;              
                     _flow[pointer] = saturated_amount;
                     _flow[_reverse[pointer]] = -saturated_amount;
      //               assert(abs<T>(this->eval_flow_saturate_arc(node,j,target)-_flow[pointer]) < _epsilon_flow);
      //               cout << "Push " << delta << endl;
                  }
               } else {
                  if (delta > _excess[node]) {
//                     cout << "Push " << _excess[node] << endl;
                     _flow[pointer] += _excess[node];
                     _flow[_reverse[pointer]] -= _excess[node];
                     _excess[child]+=_excess[node];
                     _excess[node]=0;
      //               assert(this->eval_flow_derivative_arc(node,j)-target <= _epsilon_flow);
                  } else {
                     _excess[node] -=delta;
                     _excess[child]+=delta;              
                     _flow[pointer] = saturated_amount;
       ...
       //              cout << "Push " << delta << endl;
               ...
               if (_excess[node] < _epsilon_flow) break;
            }
         } else {
            //   max_cmp_cost=MAX(max_cmp_cost,_prices[node]-reduced_cost); // check for eps
         }
      }
   }
//   cout << num_pushes-old_pushes << endl;
   // relabel
   if (_excess[node] > _epsilon_flow) {
      num_relabels++;
      // _prices[node]=max_cmp_cost-eps;
      _prices[node]-=eps;
      _list_active.push_front(node);
...
template<typename T>
T inline DoubleMinCostFlow<T>::check_reduced_costs() const {
   T eps=0;
   for (int node = 0; node<_n; ++node) {
      for (int j = 0; j<_num_arcs[node]; ++j) {
         ...
         if (_capacity[pointer] > 0) {
            ...
            const T reduced_cost=this->eval_flow_derivative_arc(node,j)-target;
            if (reduced_cost > eps) {
               if (_flow[pointer] > _epsilon_flow) eps=MAX(reduced_cost,eps);
            } else if (reduced_cost < -eps) {
               if (cap_residual > _epsilon_flow) eps=MAX(-reduced_cost,eps);
            }
         }
      }
   }
   //cout << "Epsilon optimality conditions: " << eps << endl;
   //T total=0;
   //for (int i = 0; i<_n; ++i) total+=abs<T>(_excess[i]);
   //cout << "Residual flow: " << total << endl;
   //flush(cout);
   return eps;
};
...
template <typename T>
void inline DoubleMinCostFlow<T>::saturate_arc(const int node, const int arc) {
   const int pointer=_pr_node[node]+arc;
   if (_capacity[pointer] <= 0) return;
   const int child=_children[pointer];
   const T target=_prices[child]-_prices[node];
   const T reduced_cost=this->eval_flow_derivative_arc(node,arc)-target;
   if (reduced_cost < -_epsilon_flow) {
      const T cap_residual=_capacity[pointer]-_flow[pointer];
      if (cap_residual > _epsilon_flow) {
   ...
   } else if (reduced_cost > _epsilon_flow && _flow[pointer] > _epsilon_flow) {
      const T saturated_amount = this->eval_flow_saturate_arc(node,arc,target);
      if (saturated_amount > 0) {
         const T delta=saturated_amount-_flow[pointer];
         _excess[node]-=delta;
         _excess[child]+=delta;
         _flow[pointer] = saturated_amount;
         ...
         _excess[node]+=_flow[pointer];
         _excess[child]-=_flow[pointer];
         _flow[pointer] = 0;
         _flow[_reverse[pointer]] =0;
      }
   }
};

template <typename T>
void inline DoubleMinCostFlow<T>::add_flow(const int node, const int num_arc, const T flow) {
   const int nu=_pr_node[node]+num_arc; 
   _flow[nu]+=flow;
   _flow[_reverse[nu]]-=flow;
};
...
template <typename T>
void inline DoubleMinCostFlow<T>::st_flow_decomposition_dag(List<Path<T>*>& decomposition, const int s, const int t) const {
//   cout << "Decomp" << endl;
//   flush(cout);
//   for (int i = 0; i<20; ++i) {
//      cout << i << " " << _flow[_pr_node[i]] << endl;
//   }
//   cout << endl;

   const int pr_begin=_pr_node[s];
   const int pr_end=_pr_node[s]+_num_arcs[s];
   BinaryHeap<T> heap(_n);
   int * sj_arcs = new int[_n];
   for (int pointer = pr_begin; pointer<pr_end; ++pointer) {
      if (_capacity[pointer] > _epsilon_flow && _flow[pointer] > _epsilon_flow) {
         heap.insert(_children[pointer],-_flow[pointer]);
         sj_arcs[_children[pointer]]=pointer;
      }
   }
   while (!heap.is_empty()) {
      ...
      while (node != t) {
         ...
         flow=MIN(flow,max_flow);
         if (flow <= _epsilon_flow) 
            break;
         pointers.push_back(max_pointer);
//         if (max_pointer==node) break;
         node=_children[max_pointer];
      }
      if (flow > _epsilon_flow) {
         for (const_iterator_int it = pointers.begin(); it != pointers.end(); ++it) {
         ...
         decomposition.push_back(path);
         if (init_flow-flow > _epsilon_flow) {
            heap.insert(init_node,-init_flow+flow);
         }
      } else {
         delete(path);
...
template <typename T>
void inline DoubleMinCostFlow<T>::st_flow_decomposition_dag2(List<Path<T>*>& decomposition, const int s, const int t, const int mode) const {
   if (mode==1) return this->st_flow_decomposition_dag(decomposition,s,t);
   const_cast<DoubleMinCostFlow<T>*>(this)->topological_sort(false);

   T* distances = new T[_n];
   T* unit_flows = new T[_n];
   int* lengths= new int[_n];
   int* prec = new int[_n];
   int* prec_pointer = new int[_n];

   const int pr_begin=_pr_node[s];
   const int pr_end=_pr_node[s]+_num_arcs[s];
   T total_flow=0;
   for (int pointer = pr_begin; pointer<pr_end; ++pointer) 
      if (_capacity[pointer] > _epsilon_flow)
         total_flow+=_flow[pointer];

   while (total_flow > _epsilon_flow) {
      Path<T>* path = new Path<T>();
      T& longest_flow = path->flow_int;
      list_int& list_path = path->nodes;
      list_int pointers;

      for (int i = 0; i < _n; ++i) prec[i]=-1;
      for (int i = 0; i < _n; ++i) prec_pointer[i]=-1;
      ...
      for (int i = 0; i < _n; ++i) {
         ...
         const int pr_end=_pr_node[node]+_num_arcs[node];
         for (int pointer = pr_begin; pointer<pr_end; ++pointer) {
            if (_flow[pointer] > _epsilon_flow && _capacity[pointer] > 0) {
               const int child = _children[pointer];
               const T new_unit=MIN(unit_flows[node],_flow[pointer]);
               const T new_cost= mode==2 ? new_unit*(0.5*(lengths[node]+1)+1) : new_unit;
               if (distances[child] < new_cost) {
                  distances[child]=new_cost;
                  lengths[child]=lengths[node]+1;
                  unit_flows[child]=new_unit;
      //            cerr << child << " " << new_cost << endl;
        //          stop();
                  prec[child]=node;
                  prec_pointer[child]=pointer;
               }
            }
         }
      }
...
//      flush(cerr);
      ...
      while (current != -1) {
         list_path.push_front(current);
         if (prec_pointer[current] == -1) break;
         _flow[prec_pointer[current]]-=longest_flow;
         _flow[_reverse[prec_pointer[current]]]+=longest_flow;
         current=prec[current];
      }
      total_flow -= longest_flow;
 //     cerr << longest_flow << endl;
 //     cerr << "total " << total_flow << endl;
      if (longest_flow > _epsilon_flow) {
         decomposition.push_back(path);
      } else {
         delete(path);
         break;
...
template <typename T>
T inline DoubleMinCostFlow<T>::cost_shortest_path_in_dag(list_int& list_path) {
   if (!_topologically_sorted) this->topological_sort();
   T* distances = new T[_n];
   int* prec = new int[_n];
   for (int i = 0; i < _n; ++i) prec[i]=-1;
   ...
   for (int i = 0; i < _n; ++i) {
      ...
      for (int pointer = pr_begin; pointer<pr_end; ++pointer) {
         if (_capacity[pointer] > 0) {
            const int child = _children[pointer];
            const T new_cost= distances[node] + _cost[pointer];
            if (distances[child] > new_cost) {
               distances[child]=new_cost;
               prec[child]=node;
            }
...
template <typename T>
void inline DoubleMinCostFlow<T>::scale_costs(const T scal) {
   for (int i = 0;i<_maxm; ++i) {
      _cost[i]*= scal;
   }
};
...
template <typename T>
void inline DoubleMinCostFlow<T>::price_update(const T eps) {
//   cout << "Start price update" << endl;
   _time2.start();
   T* rank = new T[_n];
   bool* scanned = new bool[_n];
   bool* temp_scanned = new bool[_n];
   BinaryHeap<T> heap(_n);
   memset(scanned,false,_n*sizeof(bool));
   memset(temp_scanned,false,_n*sizeof(bool));
   T total_excess=0;
   for (int i = 0; i<_n; ++i) {
      if (_excess[i] < -_epsilon_flow) {
         rank[i]=0;
         temp_scanned[i]=true;
         heap.insert(i,0);
         total_excess-=_excess[i];
   ...
   while (!heap.is_empty()) {
      ...
      scanned[node]=true;
      if (_excess[node] > 0) total_excess-=_excess[node];
      if (total_excess < _epsilon_flow) break;
      for (int j = 0; j<_num_arcs[node]; ++j) {
         const int pointer=_pr_node[node]+j;
         const int child = _children[pointer];
         const T cap_residual=_capacity[_reverse[pointer]]-_flow[_reverse[pointer]];
         if (!scanned[child] && cap_residual > _epsilon_flow) {
            const T target=_prices[child]-_prices[node];
            const T reduced_cost=this->eval_flow_derivative_arc(node,j)-target;
            if (reduced_cost >= 0) {
               if (temp_scanned[child]) {
               ...
               const T new_rank=rank[node]-reduced_cost;
               if (temp_scanned[child]) {
                  if (new_rank < rank[child]) {
                     rank[child]=new_rank;
                     heap.decrease_key(child,rank[child]);
...
//   cout << "Stop price update" << endl;
   _time2.stop();
};

template <typename T>
bool inline DoubleMinCostFlow<T>::topological_sort(const bool admissible, bool* admiss_node, T* reduced_costs) {
   const bool extern_admiss_node=admiss_node != NULL;
   const bool extern_reduced_costs=reduced_costs != NULL;
   int* indegree = new int[_n];
   for (int i = 0; i<_n; ++i) indegree[i]=0;
...
template <typename T = double> class DoubleGraphPath {
   public:
      DoubleGraphPath() { _n=0; _m=0; _solver=NULL; _max_capacity=MAX_CAPACITY; _init_prices=NULL; _mode_decomposition=1;};
      ~DoubleGraphPath() { delete(_solver); };

      void init_graph(const GraphPathStruct<T>& graph);
      T eval_conv(const T* input, List<Path<T>*>* decomposition = NULL, const bool alt_norm = false);
      T eval_dual_norm(const T* variables, list_int* path_out = NULL, const bool alt_norm = false);
      void proximal_conv(T* variables,const T lambda, const bool alt_norm = false);
      void sep_costs_conv(T* variables,const T lambda, const loss_flow loss, List<Path<T>*>* decomposition = NULL, const T* weights=NULL, const T delta = 0, const T tol = TOL_EPS);
      int n() const { return _n; };

   protected:
      void flow_decomposition(List<Path<T>*>& decomposition,const int mode=1) const;

   private:
      int _n;
      int _m;
      int _mode_decomposition;
      DoubleMinCostFlow<T>* _solver;
...
template <typename T>
void DoubleGraphPath<T>::init_graph(const GraphPathStruct<T>& graph) {
   _max_capacity=graph.max_capacity;
   _n=graph.n;
   _m=graph.m;
   _mode_decomposition=graph.mode_decomposition;

   int num_nodes=_n*2+2;
   int* num_arcs=new int[num_nodes];
   const int node_s=2*_n;
   const int node_t=2*_n+1;
   num_arcs[node_s]=1;  
   num_arcs[node_t]=1;
...
//   cout << graph.epsilon_flow << endl;
   _solver=new DoubleMinCostFlow<T>(num_nodes,num_arcs,graph.epsilon_flow);

   _init_weights=graph.weights;
   _init_start_weights=graph.start_weights;
   _init_stop_weights=graph.stop_weights;
   _init_prices=graph.prices;

   // s-t nodes
   _solver->set_node_s(node_s);
   _solver->set_node_t(node_t);
   _solver->add_edge(node_s,node_t,0,MAX_DEMAND);

   // j-j' connexions
   for (int i = 0; i<_n; ++i) {
      _solver->add_edge(i,i+_n,0,_max_capacity);
   }
   // s-j connexions
   for (int i = 0; i<_n; ++i) {
      if (!isinf(graph.start_weights[i])) {
         _solver->add_edge(node_s,i,graph.start_weights[i],_max_capacity);
      }
   }
   // j'-t connexions
   for (int i = 0; i<_n; ++i) {
      if (!isinf(graph.stop_weights[i])) {
         _solver->add_edge(i+_n,node_t,graph.stop_weights[i],_max_capacity);
      }
   }
   // other connexions
   for (int i = 0; i<_n; ++i) {
      for (int j= graph.jc[i]; j<graph.jc[i+1]; ++j) {
         _solver->add_edge(i+_n,graph.ir[j],graph.weights[j],_max_capacity);
      }
   }
   delete[](num_arcs);
};
...
T DoubleGraphPath<T>::eval_conv(const T* input, List<Path<T>*>* decomposition, const bool alt_norm) {
   ...
   if (alt_norm) {
      for (int i = 0; i<_n; ++i) 
         _solver->set_edge(i,0,0,_max_capacity);
   }
   if (decomposition) {
      for (int i = 0; i<_n; ++i) {
...
template <typename T>
void DoubleGraphPath<T>::flow_decomposition(List<Path<T>*>& decomposition,const int mode) const {
   _solver->set_edge(2*_n,0,0,0);
   _solver->st_flow_decomposition_dag2(decomposition,2*_n,2*_n+1,mode);
   _solver->set_edge(2*_n,0,0,MAX_DEMAND);  // to prevent dummy s-t path from being selected
   for (ListIterator<Path<T>*> it_path = decomposition.begin(); it_path != decomposition.end(); ++it_path) {
      list_int new_nodes;
      for (const_iterator_int it = it_path->nodes.begin(); it != it_path->nodes.end(); ++it) {
         if (*it <_n) new_nodes.push_back(*it);
      }
      it_path->nodes.clear();
      it_path->nodes.fusion(new_nodes);
      it_path->flow =it_path->flow_int;
   }
};

template <typename T>
T DoubleGraphPath<T>::eval_dual_norm(const T* variables, list_int* path_out, const bool alt_norm) {
   T tau=T(1.0);
   list_int path;
   bool exit_loop=false;
   bool first=true;
...
//      flush(cout);
   ...
   _solver->set_edge(2*_n,0,0,MAX_DEMAND);
   for (int i = 0; i<_n; ++i) {
      _solver->set_edge(i,0,0,_max_capacity);
   }
   if (path_out) {
...
template <typename T>
void DoubleGraphPath<T>::proximal_conv(T* variables, const T lambda, const bool alt_norm) {
   for (int i = 0; i<2*_n; ++i) 
      _solver->set_demand(i,0);
   _solver->scale_costs(lambda);
   for (int i = 0; i<_n; ++i) 
...
//      variables[i]= variables[i] >= 0 ? flow : -flow; 
   ...
   for (int i = 0; i<_n; ++i) 
      _solver->set_edge(i,0,0,_max_capacity,FLOW_LINEAR);
   _solver->scale_costs(T(1.0)/lambda);
};

template <typename T>
void DoubleGraphPath<T>::sep_costs_conv(T* variables,const T lambda, const loss_flow loss,  List<Path<T>*>* decomposition, const T* weights, const T delta, const T tol) {
   price_heuristic=false; // deactivate the price heuristic which turns out to be useless
   for (int i = 0; i<2*_n; ++i) 
      _solver->set_demand(i,0);
   _solver->scale_costs(lambda);
   ...
   _solver->set_total_demand(total_demand);
   if (_init_prices) _solver->set_prices(_init_prices);

   _solver->solve_min_cost(tol,_init_prices == NULL);

   if (_init_prices) _solver->get_prices(_init_prices);

   for (int i = 0; i<_n; ++i) 
      variables[i]=_solver->get_flow(i,0);
   
   for (int i = 0; i<_n; ++i) 
      _solver->set_edge(i,0,0,_max_capacity,FLOW_LINEAR);
   _solver->scale_costs(T(1.0)/lambda);
   if (decomposition) {
      this->flow_decomposition(*decomposition,_mode_decomposition);
   }
