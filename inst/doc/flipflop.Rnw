...
what the transcript abundances are.
...
tmp <- readLines(data.file)
@

These data correspond to the alignments of 1000 single-end reads of 125 base-pair long
...
is based on the \texttt{processsam} function of the \texttt{isolasso} software.
...
are lists \Robject{transcripts} and \Robject{abundances}. 
...
Elements of \Robject{abundances} are vector whose length is the
...
abundances <- ff.res$abundances[[1]]
print(transcripts)
print(abundances)
@
...
The transcript information from the GTF file can be easily extracted using the \Rfunction{makeTranscriptDbFromGFF} function
...
how to create a 
\Robject{TranscriptDb} object from the GTF file, 
