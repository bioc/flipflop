refitSamples <- function(count.samples, path2fit, betasum, NN, len, delta, iterpoisson, tolpoisson, verbosepath){

   if(ncol(path2fit)>0){
      Z.samples <- lapply(1:ncol(count.samples), FUN=function(jj) as.matrix(spams_flipflop.multLeftDiag(path2fit, NN[jj]*len/1e9)))
      beta.apos <- sapply(1:ncol(count.samples), FUN=function(ind.c){
      ...
      if(!is.matrix(beta.apos)){
         beta.apos <- as.matrix(beta.apos)
      } else {
         beta.apos <- t(beta.apos)
      }
      if(verbosepath){
